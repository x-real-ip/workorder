...
def login_webpage(username, password):
    username_field = setup_driver.driver.find_element(By.TAG_NAME, 'input')
    username_field.send_keys(username)
    next_button = setup_driver.driver.find_element(By.TAG_NAME, 'button')
    next_button.click()
    password_field = setup_driver.driver.find_element(By.TAG_NAME, 'input')
    password_field.send_keys(password)
    login_button = setup_driver.driver.find_element(
...
def open_workorder(date):
    try:
        order = WebDriverWait(setup_driver.driver, 3).until(EC.presence_of_element_located(
            (By.XPATH, "//div/p[contains(text(),'{}') and preceding-sibling::h6[contains(text(),'dienst') or contains(text(),'Weekenddienst') or contains(text(),'Operator') or contains(text(),'motorkap')]]".format(date))))
        order.click()
    except Exception:
...
def fill_in_form(start_hour, start_minute, end_hour, end_minute):
    ...
    end_minutes = setup_driver.driver.find_element(
...
def send_workorder():
    try:
        actions = ActionChains(setup_driver.driver)
